...
function getBatchMetadata(batchName) {
  try {
     const CACHE_KEY = 'batch_metadata_' + batchName;
     const cachedResult = getCachedData(CACHE_KEY);
     if (cachedResult) return cachedResult;
     ...
     if (!sheet) return { success: false, message: 'Sheet not found' };
      ...
      if (count > 0) {
        ...
        if (validationIdx === -1) {
             validationIdx = activeHeaders.findIndex(h => 
                ...
                !h.equals('KETERANGAN') && // Avoid generic notes
                !h.includes('SURAT')
             );
        }
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (validationIdx !== -1) {
                 const statusVal = validationValues[i][0] ? validationValues[i][0].toString().toLowerCase() : '';
      ...
      setCachedData(CACHE_KEY, finalResult, 900); // Cache for 15 minutes
